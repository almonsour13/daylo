...
export default function ActivityCard({ activity }: { activity: Activity }) {
    ...
    return (
        <View className="relative p-4 bg-white rounded-2xl overflow-hidden">
            {isDisbled && (
                <View className="absolute z-1 top-0 bottom-0 left-0 right-0 bg-white opacity-85" />
            )}
            <ColumnView className="gap-2">
                <RowView className="justify-between">
                    <RowView>
                        <View className="px-3 h-8 rounded-full flex-row gap-2 justify-center items-center bg-black">
                            <Feather name="briefcase" size={12} color="white" />
                            <Text className="text-sm text-white">Work</Text>
                        </View>
                        <View className="px-4 h-8 rounded-full justify-center items-center bg-black">
                    ...
                    <RowView>
                        <View className="h-8 aspect-square rounded-full justify-center items-center bg-black">
                            <Feather
                                name={
                                    isNotificationEnabled ? "bell" : "bell-off"
                                }
                                size={12}
                                color="white"
                            />
                ...
                <RowView className="justify-between items-start">
                    <Text className="text-3xl text-nowrap tracking-wide font-normal flex-1">
                        {activity.name}
                    </Text>
                </RowView>
                ...
                <RowView className="justify-between items-center">
                    <RowView className="">
                        {!isUpcoming && (
                            <RowView className="h-8 px-4  rounded-full items-center bg-black">
                                <Feather
                                    name="calendar"
                                    size={12}
                                    color="white"
                ...
                {isDisbled && (
                    <RowView className="z-2 justify-between items-center">
